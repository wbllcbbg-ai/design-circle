...
type HistoryItem = {
  id: string
  target_type: "case" | "article"
  target_id: string
  created_at: string
  case?: { title: string; cover_url: string; style: string; area: number }
  article?: { title: string; cover_url: string; category: string }
}
    ...
    return (
      <div className="bg-white dark:bg-zinc-900 min-h-screen">
        <div className="flex items-center gap-2 px-4 h-12 border-b border-zinc-100 dark:border-zinc-800">
          ...
          <h1 className="text-sm font-medium">浏览历史</h1>
        </div>
        <div className="flex flex-col items-center justify-center py-20 text-zinc-400">
          <p className="text-sm">登录后查看浏览历史</p>
    ...
    <div className="bg-white dark:bg-zinc-900 min-h-screen">
      ...
      {history.length === 0 ? (
        ...
        <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
          {history.map((item) => {
            const title = item.target_type === "case" ? item.case?.title : item.article?.title
            const href = item.target_type === "case" ? `/cases/${item.target_id}` : `/articles/${item.target_id}`
            return (
              <Link key={item.id} href={href} className="flex items-center gap-3 px-4 py-3 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium line-clamp-1">{title || "未知内容"}</p>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800 text-zinc-400">
                      {item.target_type === "case" ? "案例" : "文章"}
                    </span>
